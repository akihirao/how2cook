...
bkfrac_true <- 0.3 #true initial depletion rate if gradual type of simulation
...
sim.dif.pdm <- function(type, range_year, K, r, n, q, sigma_pro, sigma_obs){
  if(type=="F_unifrom"){ # type of "F_uniform": F follows a uniform distribution
    years <- seq(range_year[1],range_year[2],by=1)
    nyears <- length(years)
    
    F_vec <- B_vec <- C_vec <- NULL
    B_vec[1] <- K*runif(1,0,1) # determining initial biomass
    bkfrac <- B_vec[1]/K # initial depletion rate
    F_vec[1] <- runif(1,0,1) # determining initial F
  ...
  }else if(type=="F_norm"){ # type of "F_rnorm": F follows a normal distribution
    F0 <- bkfrac_true
    years <- seq(range_year[1],range_year[2],by=1)
    nyears <- length(years)
    ...
    F_vec <- B_vec <- C_vec <- NULL
    B_vec[1] <- K*runif(1,0,1) # determining initial biomass
    F_vec[1] <- F0 # initial depletion rate
...
k <- 1
while (k <= no_sim){
  sim_data <- sim.dif.pdm("F_norm",range_year,K_true, r_true, n_true,q_true,sigma_pro, sigma_obs)
  inp <- sim_data[[1]]
...
param_summary <- param %>% group_by(time_step) %>% dplyr::summarise(r_median=format(round(median(r),2),nsmall=2), K_median=format(round(median(K),1),nsmall=1),m_median=format(round(median(m),1),nsmall=1),sdb_median=format(round(median(sdb),3),nsmall=3),Bmsys_median=format(round(median(Bmsys),1),nsmall=1),Fmsys_median=format(round(median(Fmsys),3),nsmall=3),MSYs_median=format(round(median(MSYs),1),nsmall=1),bkfrac_median=format(round(median(bkfrac),1),nsmall=1),r_CV=format(round(sd(r)/mean(r),3),nsmall=3), K_CV=format(round(sd(K)/mean(K),3),nsmall=3),m_CV=format(round(sd(m)/mean(m),3),nsmall=3),sdb_CV=format(round(sd(sdb)/mean(sdb),3),nsmall=3),Bmsys_CV=format(round(sd(Bmsys)/mean(Bmsys),3),nsmall=3),Fmsys_CV=format(round(sd(Fmsys)/mean(Fmsys),3),nsmall=3),MSYs_CV=format(round(sd(MSYs)/mean(MSYs),3),nsmall=3),bkfrac_CV=format(round(sd(bkfrac)/mean(bkfrac),3),nsmall=3))
print(param_summary)
...
params_violinplot <- grid.arrange(m_violinplot, r_violinplot, K_violinplot,ncol=1)
...
BlBmsy_FlFmsy_summary <- param %>% group_by(time_step) %>% dplyr::summarise(biase_BlBmsy_median=format(round(median(biase_BlBmsy,na.rm=TRUE),2),nsmall=2), biase_FlFmsy_median=format(round(median(biase_FlFmsy,na.rm=TRUE),2),nsmall=2))
...
bk_sdb_labels = data.frame(x =0.4, y = 3.0, label = corr_eqn(param$sdb, param$bkfrac))
...
K_bk_labels = data.frame(x =6000, y = 3.0, label = corr_eqn(param$K, param$bkfrac))
