...
#euler_series <- c(1/64,1/16,1/4,1)
euler_series <- c(1/16,1/4,1)
no_euler_series <- length(euler_series)

data(pol)
inp_albacore <- pol$albacore
inp_albacore <- check.inp(inp_albacore)
res_albacore <- fit.spict(inp_albacore)

nyears <- length(inp_albacore$timeC)

#Simulate a specific number of observations for each of euler 
for(i in 1:1){
#for(i in 1:no_euler_series){
  inp <- list()
  inp$dteuler <- euler_series[i]
  inp$ini <- res_albacore$inp$ini
  inp$ini$logF <- NULL
  inp$ini$logB <- NULL
  set.seed(1)
  sim <- sim.spict(inp, nobs=nyears)
